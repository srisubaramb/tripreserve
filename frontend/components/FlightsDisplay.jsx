...
import { removeFlightFromFav, saveFlightToFav } from "../store/flightSlices";
import { airlineCodeToDetails, currencyCodeToSymbol } from "../utils/utils";
    ...
    <div className="flex flex-col gap-y-2 w-[90%] mx-auto my-2">
      {isDataThere &&
        data.map((flight, index) => {
          ...
          if (!airlineDetails) {
            console.warn("Unknown airline code:", flight.airline);
          }
          const depatureDateTime = new Date(flight.departure_at);
          const arivalTime = new Date(depatureDateTime);
          arivalTime.setMinutes(
            depatureDateTime.getMinutes() + flight.duration,
          );
          ...
          return (
            <div
              key={index}
              className="flex gap-x-2 justify-center px-2 py-5 drop-shadow-md rounded-2xl bg-white"
            >
              <div>
              ...
              <div className="flex flex-col flex-1 px-3 gap-y-1 ">
                <div className="flex justify-between">
                  ...
                  <p className="opacity-60">
                    {isNaN(hours) ? "XX " : hours}hrs{" "}
                    {isNaN(minutes) ? "XX" : minutes}Min
                  </p>
                  <p>{arivalTimeText}</p>
                </div>
              ...
              <div className="flex flex-col gap-y-1 justify-center items-center ">
                <p className="text-2xl">
                  {currencyCodeToSymbol(currency, flight.price)}
                </p>
                <a
                  href={redirectUrl}
                  target="_blank"
      ...
      {!isDataThere && (
        <div className="flex justify-center">
          <div className="w-fit text-center text-xl bg-red-400 text-white p-1 px-2 rounded-3xl">
            Flight data is currently not cached for these dates, or no flights
            are available.
          </div>
        </div>
      )}
