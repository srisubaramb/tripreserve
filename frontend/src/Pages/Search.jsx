...
import { setCurrency } from "../../store/flightSlices";
import { filterAriports, getFlights } from "../../utils/utils";
import flightsAvaliable from "../assets/defaultFlights.json";
function Search() {
  const [searchData, setSearchData] = useState({
    from: { name: "Chennai", iata: "MAA" },
    to: { name: "Madurai", iata: "IXM" },
    depatureDate: "",
    returnDate: "",
    oneWay: true,
  });
  ...
  // Get current month in YYYY-MM format to restrict past date selection
  const currentMonth = new Date().toISOString().slice(0, 7);
  const dispatch = useDispatch();
  //Maintain the current Suggestion of the city using activeField
  const currentSuggestion =
    activeField === "from"
      ? filterAriports(searchData.from.name, airports)
      : activeField === "to"
        ? filterAriports(searchData.to.name, airports)
        : [];
  useEffect(() => {
    fetch("/airports.json")
  ...
  function handleDropDownSelect(airpot) {
    setSearchData((perv) => {
      return {
        ...perv,
        [activeField]: { name: airpot.city, iata: airpot.iata },
      };
    });
    setActiveField(null);
  }
      ...
      <div className="max-w-[96%] mx-auto mt-2 py-1 px-1">
        ...
        <div className=" bg-white rounded-xl drop-shadow-sm">
          ...
          >
            ...
            <svg
              className="rotate-90 self-center -my-3 md:rotate-0 "
              viewBox="0 0 200 200"
              width="1.25em"
              height="1.25em"
            ...
            <div className="flex flex-col">
              <label className="text-xs text-gray-500 mb-1 ml-1 font-semibold">
                Departure
              </label>
              <input
                type="month"
            ...
            <input
              type="submit"
              value="Search"
              className="bg-primary rounded-xl px-3 font-spartan font-bold "
            />
          </form>
        </div>
      </div>
      ...
      {!searchData.oneWay && (
        <div className="flex justify-center">
          <div className="w-fit text-center text-xl bg-red-400 text-white p-1 px-2 rounded-3xl">
            Sorry for the inconvenience, currently return trip search is not
            supported.
          </div>
        </div>
      )}
