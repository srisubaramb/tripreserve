	...
	<>
			...
			<header className={`flex flex-col z-10  w-full justify-between mb-2 md:h-auto md:flex-row ${isMenuOpen ? 'h-screen' : 'h-auto'}`}>
					...
					<ul className="flex   flex-col  gap-y-3   justify-center md:flex-1 md:flex-row md:self-end md:gap-x-4">
						...
						<li title='Search Flights'><Link to={'/search-flights'} className={navLinkStyle}>Flights</Link></li>
						<li><Link to={'/about'} className={navLinkStyle}>About Us</Link></li>
					</ul>
					<ul className="flex gap-y-3 pb-4 flex-col md:flex-row md:items-center md:gap-x-3">
						<li><Link to="/help"  className={navLinkStyle}>Help</Link></li>
						...
						{!isAuth && <Link to="/signup" className={navLinkStyle}>Sign Up</Link>}
						{!isAuth && <Link to="/login" className={navLinkStyle}>Login</Link>}
					</ul>
					</nav>
			</header>
	</>
